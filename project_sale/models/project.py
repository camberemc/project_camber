...
class ProjectExtented(models.Model):
    ...
    def complete_project(self):
        if self.retention_date:
            ...
            raise UserError('Retention date not Added. Please fill out the retention date and close the project.')
...
class ProjectOderLine(models.Model):
    ...
    intented_date = fields.Date(string='Intended Date', track_visibility='onchange')
    ...
    total_qty_delivered = fields.Float(string='Delivered Qty', compute='_compute_qty', track_visibility='onchange')
    qty_return_quantity = fields.Float(string='Return Qty', compute='_compute_qty', track_visibility='onchange')
    ...
    project_return_ids = fields.One2many('stock.inventory.return.line', 'project_order_line_id',
    ...
    def _compute_qty(self):
        for record in self:
            ...
            record.total_qty_delivered = sum(record.project_delivery_ids.mapped('qty'))
            record.qty_on_hand = record.product_id.qty_available
            record.qty_return_quantity = sum(record.project_return_ids.mapped(('quantity')))
            if record.qty_return_quantity:
                record.qty_delivered = record.qty_delivered - record.qty_return_quantity
    ...
    def _check_quantity_return(self):
        for record in self:
            if record.qty_added_inventory_return > record.qty_delivered:
                raise Warning(
    ...
    def create_delivery_return(self):
        record_ids = self._context.get('active_ids')
        if record_ids:
            selected = self.env['project.project.orderline'].browse(
                record_ids)
            for record in selected:
                if not record.project_id.is_approved:
                    # if record.project_id.state != 'approved':
                    raise UserError('You have No Permission to Create Inventory Return')
            action = self.env.ref('project_sale.action_project_inventory_return').read()[0]
            action['context'] = dict(
                self.env.context,
                default_orderline_ids=selected.ids,
...
class ProjectDeliveryIndent(models.Model):
    ...
    cost = fields.Monetary(string='Total Cost', compute='_compute_calcs', store=True)
    ...
    total_qty_delivered = fields.Float(string='Delivered Qty')
    qty_return_quantity = fields.Float(string='Return Qty', track_visibility='onchange')
    qty_added_inventory_return = fields.Float(string='Added +')
    qty_balance_delivery = fields.Float(string='Balance Delivery')

    @api.onchange('qty_added_inventory_return')
    def _check_quantity_return(self):
        for record in self:
            if record.qty_added_inventory_return > record.total_qty_delivered:
                raise Warning(
                    _('You cannot create return delivery with excess delivered quantity'))
    ...
    def onchange_qty_added_delivery(self):
        for record in self:
            if record.qty_added_delivery > (record.product_qty - record.total_qty_delivered):
                raise UserError('Quantity should be lesser than total quantity')
    ...
    @api.depends('delivered_qty', 'unit_cost')
    def _compute_calcs(self):
        for record in self:
            record.cost = record.delivered_qty * record.product_cost
            # record.total_qty_delivered = record.delivered_qty
            # if record.qty_return_quantity:
            #     record.total_qty_delivered = record.total_qty_delivered - record.qty_return_quantity
            # record.qty_balance_delivery = record.product_qty - record.total_qty_delivered
    ...
    def create_delivery_return(self):
        ...
        if record_ids:
            selected = self.env['project.delivery.indent'].browse(
                record_ids)
            action = self.env.ref('project_sale.action_project_intend_inventory_return').read()[0]
            action['context'] = dict(
                self.env.context,
                default_orderline_ids=selected.ids,
                default_project_id=self.project_id.id,
