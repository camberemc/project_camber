    ...
    def action_post(self):
        ...
        if self.expense_line_ids:
            if self.voucher_type == 'send':
                taxes_paid = self.env['account.account'].search([('name', '=', 'Taxes Paid')], limit=1)
                account_lines = []
            ...
            if self.voucher_type == 'receive':
                taxes_received = self.env['account.account'].search([('name', '=', 'Taxes Received')], limit=1)
                account_lines = []
                account_lines.append([0, 0, {
                    'name': self.name,
                    'account_id': self.journal_id.payment_credit_account_id.id,
